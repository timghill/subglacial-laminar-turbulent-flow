...
% Adjust BCs for trough outlet condition
dist = abs(dmesh.tri.nodes(:, 2) - 12.5e3);
dist(dmesh.tri.nodes(:, 1)>0) = 101e3;
[min_dist, pos] = min(dist);

% Adjust dirichlet conditions to zero-flux
dmesh.tri.bmark(dmesh.tri.bmark==1) = 2;
% Set dirichlet condition only at outlet
dmesh.tri.bmark(pos) = 1;

% Adjust dirichlet conditions to zero-flux on edges
dmesh.tri.bmark_edge(dmesh.tri.bmark_edge==1) = 2;
...
pin.ice_thickness = make_anon_fn('@(xy, time) double(ice_thickness_trough2(xy, time))');
